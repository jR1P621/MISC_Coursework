...
public class Player extends Character {
	...
	public Player() {
		super("Human", 1, 1, 20);  // Start in 1,1  with 20 HP
		items = new ArrayList<>();
	}
	...
	private boolean attackOpponent(Room room, int newX, int newY, Character[] opponents) {
		...
		for (Item i : items) {
			...
			// If the player has things on his fingers
			if (i.name.toLowerCase().contains("fingers"))
				if (maxDamage < 30) {
					maxDamage = 30;
					weapon = i.name;
				}
		...
		for (Character opponent : opponents)
			if ((opponent.x == newX) && (opponent.y == newY)) {
				...
				System.out.println(name + " attacks " + opponent.name + " with " + weapon + " for "
						+ damage + " damage!");
				opponent.hp -= damage;
