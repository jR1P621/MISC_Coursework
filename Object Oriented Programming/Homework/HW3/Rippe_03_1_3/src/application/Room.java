...
public class Room {
	...
	protected String doorSprite;
	protected Color floorColor;

	public Room(String roomName, int width, int height, ArrayList<Item> items, ArrayList<Character> monsters,
			String wallSprite, String doorSprite, Color floorColor) {
		this.width = width;
		this.height = height;
		...
		if (items != null) {
			for (Item i : items) {
				...
				if (i instanceof RoundDoorItem)
					((RoundDoorItem) i).setSprite(doorSprite);
				this.items.add(i);
			}
		}
